...
def main():
    ...
    data_json_dir = "../4D-Data/VSTI-Bench/vstibench_train"
    ...
    if os.path.exists(data_json_dir):
        for file_name in sorted(os.listdir(data_json_dir)):
            if file_name.endswith(".json"):
                try:
                    with open(os.path.join(data_json_dir, file_name), 'r') as f:
                        data = json.load(f)
                        if isinstance(data, list):
                            full_data.extend(data)
                except Exception as e:
                    if rank == 0: print(f"Error loading {file_name}: {e}")
    ...
    for item in full_data:
        ...
        if os.path.exists(vid_path):
            ...
            if len(conversations) >= 2:
                val = conversations[0]['value']
                if isinstance(val, str):
                    question = val.replace("<video>\n", "").replace("<video>", "").replace("<image>\n", "").replace("<image>", "")
    ...
    for epoch in range(num_epochs):
        ...
        for i, sample in enumerate(training_samples):
            ...
            # Visualize Teacher Outputs (Rank 0 only, first sample only)
            if rank == 0 and global_step_counter == 0:
                 vis_out_root = os.path.join("../4D-Data/vis_debug", f"epoch_{epoch}_sample_0")
                 print(f"Visualizing first sample to {vis_out_root}")
                 visualize_teacher_outputs(vis_out_root, p_m_teacher, global_step_counter)
