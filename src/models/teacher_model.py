...
class TeacherWrapper(nn.Module): 
    ...
    def get_targets(self, frames_list):
        ...
        # 4. Camray / Trajectory
        # Logic adapted from my_demo.py to handle different output formats (direct rays vs trajectory)
        if "camray_est_b9thw" in out: 
            # Direct output if available
            results["camray"] = out["camray_est_b9thw"].float().detach().cpu()
        elif "camray_est_b6thw" in out:
            # Another variant of direct output
            results["camray"] = out["camray_est_b6thw"].float().detach().cpu()
        elif "traj3d_est_b16t" in out:
            # Reconstruct rays from trajectory (matches my_demo.py logic for your model checkpoint)
            try:
                traj3d_est_b16t = out["traj3d_est_b16t"]
                B_sz, _, T_sz = traj3d_est_b16t.shape
                
                # Reshape trajectory to Matrix
                world_T_cam_b44t = traj3d_est_b16t.reshape(B_sz, 4, 4, T_sz)
                
                # Get Extrinsics (Cam -> World)
                ...
                if "traj3d_intrinsics_est_b16t" in out:
                    ...
                    intrinsics_b44t = intrinsics_est_b16t.reshape(B_sz, 4, 4, T_sz).float()
                else:
                    # Fallback if no intrinsics estimated (though dense_heads usually outputs it)
                    # Create identity/dummy or skip? 
                    # For now, let's create a dummy identity intrinsic or normalized one
                    intrinsics_b44t = torch.eye(4, device=traj3d_est_b16t.device).view(1, 4, 4, 1).repeat(B_sz, 1, 1, T_sz)
                    intrinsics_b44t[:, 0, 0, :] = 1.0 # fx
                    intrinsics_b44t[:, 1, 1, :] = 1.0 # fy
                    intrinsics_b44t[:, 0, 2, :] = 0.5 # cx
                    intrinsics_b44t[:, 1, 2, :] = 0.5 # cy

                # Normalize intrinsics (H=224, W=224 fixed in teacher)
                H_out, W_out = 224, 224
                intrinsics_norm_b44t = normalize_intrinsics(intrinsics_b44t, H_out, W_out)
                
                # Compute Plucker Rays -> (B, 6, T, H, W)
                # Ensure geometry utils are available
                if get_rays_plucker is not None:
                    ...
                    results["camray"] = camray_b6thw.float().detach().cpu()
            except Exception as e:
                print(f"Error reconstructing camray from trajectory: {e}")

        # Final check
        if "camray" not in results and "flow" not in results:
             print(f"Warning: Missing flow/camray output keys. Available keys: {list(out.keys())}")
